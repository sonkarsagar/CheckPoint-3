...
download.addEventListener("click", (e) => {
  e.preventDefault();
  axios
    .get("http://3.82.230.183:3000/expense/download", {
...
premium.addEventListener("click", (e) => {
  e.preventDefault();
  axios
    .get("http://3.82.230.183:3000/expense/premium", {
      headers: { Authorization: localStorage.getItem("token") },
    })
    .then((response) => {
      let options = {
        ...
        handler: function (response) {
          axios.post(
            "http://3.82.230.183:3000/expense/successTransaction",
            {
          ...
          location.replace("http://3.82.230.183:3000/loggedIn/premiumIndex.html");
        },
      };
      ...
      rzp1.on("payment.failed", function (response) {
        axios.post(
          "http://3.82.230.183:3000/expense/failTransaction",
          {
            order_id: options.order_id,
          },
          { headers: { Authorization: localStorage.getItem("token") } }
...
submit.addEventListener("click", (e) => {
  ...
  axios
    .post(
      "http://3.82.230.183:3000/expense",
      {
...
tBody.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("btn-danger")) {
    ...
    axios
      .delete(
        `http://3.82.230.183:3000/expense/${e.target.parentElement.parentElement.id}`
      )
      .then((res) => {})
      .catch((err) => {
        console.log(err);
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (!localStorage.getItem("token")) {
    return location.replace("http://3.82.230.183:3000/logIn/login.html");
  }
  axios
    .get("http://3.82.230.183:3000/user", {
      headers: { Authorization: localStorage.getItem("token") },
    })
    .then((result) => {
  ...
  axios
    .get(
      `http://3.82.230.183:3000/expense/pagination?page=${page}&limit=${localStorage.getItem(
        "limit"
      )}`,
      {
        headers: { Authorization: localStorage.getItem("token") },
